...
class ProcedureAuditImport implements ToCollection, WithStartRow
{
    /**
    ...
    public function collection(Collection $collection)
    {
        ...
        if (empty($row[1]) || strtoupper($row[1]) != strtoupper('Prosedur Audit')) {
        ...
        foreach ($collection as $rw => $row) {
            ...
            if (!empty($name) && !empty($aspectName) && !empty($category) && !empty($objectType) && !empty($objectName)) {
                // Check Category
