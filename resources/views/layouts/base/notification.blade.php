@if (isset($notifications) && count($notifications))
    <div class="navi navi-hover scroll my-4 user-notification-items"
        data-count="{{ auth()->user()->notifications()->wherePivot('readed_at', null)->count() }}" data-scroll="true"
        ...
        @foreach ($notifications as $item)
            <a href="{{ route('ajax.userNotificationRead', $item->id) }}"
                ...
                data-url="{{ route('ajax.userNotificationRead', $item->id) }}">
                <div class="navi-link">
    ...
    <div class="text-center">
        ...
        <a href="{{ route('setting.profile.notification') }}" class="btn btn-light-primary mb-3 base-content--replace">
            {{ __('Baca Semua') }} <i class="fas fa-angle-right ml-2"></i>
        </a>
    </div>
@else
